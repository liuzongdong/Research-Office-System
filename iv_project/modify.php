        ...
        else
        {
            ...
            if ($upload_file != "")
            {
                ...
                else
                {
                    ...
                    $sql = "update iv_project set iv_project_name = ?, iv_project_budget = ?, iv_project_file = ?, update_date = ?, iv_project_status = ? where iv_project_id = ?";
                    $prepare = $dbh -> prepare($sql);
                    $execute = $prepare -> execute(array($iv_project_name_of_institute_or_center, $iv_project_budget, $filenamekey, $update_date, "0", $_POST['id']));
                    if ($execute)
                    {
            ...
            else
            {
                ...
                $sql = "update iv_project set iv_project_name = ?, iv_project_budget = ?, update_date = ?, iv_project_status = ? where iv_project_id = ?";
                $prepare = $dbh -> prepare($sql);
                $execute = $prepare -> execute(array($iv_project_name_of_institute_or_center, $iv_project_budget, $update_date, "0", $_POST['id']));
                if ($execute)
                {
                    $response = array('status_response'  => 'success');
                    echo json_encode($response);
